...
const Club = {
  ...
  // 모임 상세 조회
  async v1FetchClubDetail(clubId: number) {
    try {
  ...
  // 모임별 신청받은 내역 조회
  async v1ApplicationList(clubId: number) {
    try {
      const url = `${prefix}/applicationList/${clubId}`;
  ...
  // 모임별 리뷰 조회
  async v1FetchClubReviews(clubId: number, page: number, size: number) {
    try {
      const url = `${prefix}/reviews/${clubId}`;
      const result = await instance.get(url, {
        params: {
          page,
          size,
